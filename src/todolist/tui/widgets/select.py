# LICENSE: https://github.com/mitosch/textual-select/blob/main/LICENSE
...
class SelectListView(ListView):
    ...
    def on_list_view_selected(self, something):
...
class SelectList(Widget):
    ...
    def select_highlighted_item(self) -> None:
        print("Select Highlighted Item: ", self.list_view.index)
...
class Select(Widget, can_focus=True):
    ...
    async def watch_value(self, value: str) -> None:
        ...
        if value is None:
            ...
            self.text = self.items[int(value)]['text']
            print("Change in Value", self.text)
            # chevron = "\u25bc"
            # width = self.content_size.width
            # text_space = width - 2
            # text = f"{self.text:{text_space}} {chevron}"
